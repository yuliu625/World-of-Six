...
def get_save_path(beta: float, price_index: int):
    base_dir_to_save = r"D:\document\code\paper\World-of-Six\results_qwen_15\results_raw"
    dir_to_save_025 = base_dir_to_save + r"/beta_025/no_guidance"
    dir_to_save_075 = base_dir_to_save + r"/beta_075/no_guidance"
    if beta == 0.25:
        return dir_to_save_025 + rf"/same_price_{price_index}"
    elif beta == 0.75:
        return dir_to_save_075 + rf"/same_price_{price_index}"
...
async def main(
    ...
    tasks = []
    for price_index in range(4, 6):
        network_effect_with_prices_list = prices_list_factory.get_same_price_list(price_index)
